...
function loupanflag($fl) {

	global $mysql;

    $row = $mysql->query("select * from `web_flag` where `flag_bm`='{$fl}'");
    return $row[0]['flag'];

}
//特色
function loupanflagtsi($ts,$fl,$nums) {
	global $mysql;
	...
	foreach($row as $list){
		if(preg_match("#".$list['flag_bm']."#", $ts) && $i<=$nums){
			$i=$i+1;
		$tsname.='<i>'.$list['flag'].'</i>';
		}
	}
    return $tsname;
}
function loupanflagtsit($ts,$fl,$nums) {
	global $mysql;
    $row = $mysql->query("select * from `web_flag` where `flag_fl`='{$fl}'");
	$i=1;
	foreach($row as $list){
		if(preg_match("#".$list['flag_bm']."#", $ts) && $i<=$nums){
			$i=$i+1;
		$tsname.='<span class="btns"><a href="javascript:;" class="b2">'.$list['flag'].'</a></span>';
		}
	}
    return $tsname;
}
...
function loupanflagts3($ts,$fl) {
	...
	foreach($row as $list){
		...
		$tsname.='<samp style=" border:'.$list['color'].' solid 1px;background:'.$list['color'].';">'.$list['flag'].'</samp>';
		}
	}
    return $tsname;
}
...
function loupanflagts8($ts,$fl,$top) {
...
function lmname($id) {
    ...
    $lpname = $mysql->query("select * from `web_srot` where `id`='{$id}' limit 0,1");
...
//if($_SERVER['HTTP_HOST']==$siteasd){
//	
//	header('HTTP/1.1 301 Moved Permanently');
//    //跳转到你希望的地址格式 
//    header('Location: http://www.'.$siteasd.'/city.html');
//	}
	...
	if (!urlhash.match("fromapp"))
		...
		if ((navigator.userAgent.match(/(iPhone|iPod|Android|ios|iPad)/i)))

			{
				...
				window.location="http://<?php echo $pingyi;?>.<?php echo $siteasd;?>/m"+window.document.location.pathname+window.location.search;
...
-->
function sq(){
	window.open('<?php echo $shangqiao;?>','','width=740,height=600,top='+(screen.height-600)/2+',left='+(screen.width-600)/2 + '');
	}
