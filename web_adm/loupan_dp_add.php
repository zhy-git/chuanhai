...
if($_GET['id']<>""){
	$rows=$mysql->query("SELECT * FROM `web_content_dp` WHERE `id`='{$_GET['id']}'");
	$infos=$rows[0];
	 if($infos==false)
	  {
		echo "已经出错!";
...
<div class="content">
  <form action="submit_save.php?action=lp_dp_add" method="post" name="myform" id="myform">
    <div class="addcust" id="client">
      <ul class="addcust_ul c">
        ...
        <li>
          <p class="lie1">
            <label>内容：</label>
            <textarea name="edit_content" style="width:940px;height:100px;"><?php echo $infos['content'];?></textarea>
