...
if($result["total"]==0){
		...
		foreach($row as $list){
			...
			$data['html'].='<tr class="tr '.$lv.'" style="">
					...
					<td style="text-align:left;">'.$lpname.'<a style="" href="loupan_news_add.php?id='.$list[id].'&pid='.$list[pid].'">'.$list[title].'</a></td>
					<td>'.$list[addtime].'</td>
					<td><a href="loupan_news_add.php?id='.$list[id].'&pid='.$list[pid].'" target="c" class="ceditor" style=" margin-right:5px;">详情</a></td>
					</tr>';
			$i++;
			}
		}
