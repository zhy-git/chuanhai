...
<?php
if($_GET['id']<>""){echo '&gt; 修改二手房信息';}else{echo '&gt; 添加二手房';}
?>
</div>
<div class="content">
  <form action="submit_save.php?action=esf_add" method="post" name="myform" id="myform">
    <div class="addcust" id="client">
      <ul class="addcust_ul c">
        ...
        <li>
          ...
          <p class="lie1">
            <label>销售状态：</label>
            <input type="radio" value="1" name="esfzt" checked="checked"/>销售中
            <input type="radio" value="2" name="esfzt" />已出售
          </p>
        </li>
         <li>
          <p class="lie1">
        ...
        <li>
          <p class="lie1">
            <label>售　　价：</label>
            <input type="text" name="tuanprice" value="<?php echo $infos['tuanprice'];?>" style="width:100px;" >万
          </p>
          <p class="lie1">
            <label>建筑面积：</label>
        ...
        <li>
          <p >
            <label>特色：</label>
            <?php
            $rowflag = $mysql->query("select * from `web_flag` where `flag_fl`='12' and `flag_st`='1' order by `flag_px` asc ");
			foreach($rowflag as $listflag){
