...
$keys=$_POST['keys'];
$lpid=$_GET['lpid'];
$rowlp=$mysql->query("SELECT * FROM `web_content` WHERE `id`='{$lpid}'");
...
$(document).ready(function() {
	...
	function clientajax(keyword) {
		...
		$.get('loupan_news_ajax.php?page='+keyword+'&keys='+keys+'&pid='+pid+'&lpid='+lpid, function(data) {
			//alert(data.html);
			//回调函数
			//data = eval('('+data+')');
			if (data.msg=='ok') {
...
<div class="cus_search2"><a href="loupan_news_add.php?pid=<?php echo $pid;?>&lpid=<?php echo $lpid;?>" class="newadd">添加楼盘信息</a><a href="loupan_list.php?pid=<?php echo $rowlp[0]['pid'];?>&page=<?php echo $_GET['page'];?>" class="back">返回楼盘列表</a></div>
</div>
