...
$(document).ready(function() {
	...
	function clientajax(keyword) {
		...
		$.get('link_ajax.php?page='+keyword+'&keys='+keys+'&pid='+pid+'&ad_id='+ad_id, function(data) {
			//alert(data.html);
			//回调函数
			//data = eval('('+data+')');
			if (data.msg=='ok') {
...
<div class="cus_search2"><a href="link_add.php?pid=<?php echo $pid;?>&cid=<?php echo $cid;?>" class="newadd">添加新信息</a></div>
</div>
<div class="cus_search c">
    ...
    广告分类：
    <?php
    $row = $mysql->query("select * from `web_srot` where `pid`='{$pid}'");
	foreach($row as $list){
	?>
     <a href="?pid=<?php echo $pid;?>&ad_id=<?php echo $list['id'];?>" <?php if($ad_id==$list['id']){ echo ' style="color:#F00"';}?>><?php echo $list['title'];?></a>　　
     <?php
	}
	 ?>
    </div>
