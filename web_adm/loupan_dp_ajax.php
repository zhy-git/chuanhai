...
function lpname($lpid) {
	global $mysql;
    $lpname = $mysql->query("select * from `web_content_dp` where `id`='{$lpid}' limit 0,1");
    return $lpname[0]['title'];
}
$page_num =12;
$offset = ($page-1)*$page_num;
$rsc = $mysql->query("select count(*) as count from `web_content_dp` {$sql}");// WHERE `adminid`='{$_SESSION['admin_id']}'
//echo "select count(*) as count from `web_content` {$sql}";
$result["total"] = $rsc[0]['count'];
$total=ceil($result["total"]/$page_num);
$row = $mysql->query("select * from `web_content_dp` {$sql} order by id desc limit $offset,$page_num");// WHERE `adminid`='{$_SESSION['admin_id']}'
...
if($result["total"]==0){
		...
		foreach($row as $list){
			...
			$data['html'].='<tr class="tr '.$lv.'" style="">
					...
					<td style="text-align:left;">'.$lpname.'<a style="" href="loupan_dp_add.php?id='.$list[id].'&pid='.$list[pid].'">'.$list[title].'</a></td>
					<td>'.$list[addtime].'</td>
					<td><a href="loupan_dp_add.php?id='.$list[id].'&pid='.$list[pid].'" target="c" class="ceditor" style=" margin-right:5px;">详情</a></td>
					</tr>';
			$i++;
			}
		}
