...
if($action=="add_sort")
{
	...
	if ($add_pid==0){
			$add_path="0";
				...
				$sql = "select * from lscydb_srot where id='".$add_pid."'";
				$exec = mysql_query($sql,$conn);
				if($rs= mysql_fetch_array($exec))
				{
					$add_path=$rs[path]."-".$add_pid;
				}
			}
	if(mysql_query("insert into lscydb_srot(title,px,pid,startus,b_url,path) values ('".$add_title."','".$add_px."','".$add_pid."','".$add_startus."','".$add_b_url."','".$add_path."')",$conn))
	{
		echo "ok";
		mysql_close($conn);
		exit();
...
if($action=="edit_srot")
{
	...
	if(mysql_query("UPDATE lscydb_srot SET title='".$edit_title."',px='".$edit_px."',startus='".$edit_startus."',b_url='".$edit_b_url."' where id=".$edit_id,$conn))
	{
		echo "ok";
		mysql_close($conn);
		exit();
...
if($action=="del_srot")
{
	...
	$sql = "select * from lscydb_content where pid='".$id."'";
	$exec = mysql_query($sql,$conn);
	if($rs= mysql_fetch_array($exec))
	{
		echo "nodel";
	...
	if(mysql_query("delete from lscydb_srot where id='".$id."'",$conn))
	{
		echo "ok";
		mysql_close($conn);
		exit();
...
{
	$id=$_POST[id];
	if($mysql->execute("DELETE FROM `client` WHERE `id`='".$id."'",$conn))
	{
	////记录日志
//		 $mysql->query("insert into `log`(`event`,`day`,`datetime`) values ('管理员：".$_SESSION['admin_real']."删除了帐号：".$edit_admin_name.",用户：".$edit_admin_real."','".date("Y-m-d",time())."','".time()."')");
	echo "ok";
