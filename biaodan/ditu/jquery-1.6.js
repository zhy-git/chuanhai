...
//修改源码,对中文字符做转码处理,后台JAVA部分对应使用RequestUtil.ajaxToString()方法来获取中文参数
for(var i in s.data) {
	if(s.data.hasOwnProperty(i)) {
		if(/.*[\u4e00-\u9fa5]+.*$/.test(s.data[i])) {
			s.data[i] = escape(escape(s.data[i]));
		}
	}
}
...
// 修改源码,对中文字符做转码处理,后台JAVA部分对应使用RequestUtil.ajaxToString()方法来获取中文参数
if(s.url.indexOf("?") > 0 && s.url.indexOf("?") < (s.url.length - 1)) {
	var queryString = "";
	var uri = "";
	queryString = s.url.substring(s.url.indexOf("?") + 1, s.url.length);
	uri = s.url.substring(0, s.url.indexOf("?") + 1);
	if(/.*[\u4e00-\u9fa5]+.*$/.test(queryString)) {
		var params = queryString.split("&");
		for(var i=0;i<params.length;i++) {
			var param = params[i].split("=");
			if(/.*[\u4e00-\u9fa5]+.*$/.test(param[1])) {
				param[1] = escape(escape(param[1]));
