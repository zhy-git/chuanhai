...
jwplayer.version = "6.10.4906", jwplayer.vid = document.createElement("video"), jwplayer.audio = document.createElement("audio"), jwplayer.source = document.createElement("source"),
function(h) {
	function c(f) {
		return function() {
			return a(f)
	...
	function l(f, a, k, m, d) {
		return function() {
			...
			if (d) k(f);
				...
				if (b && c) return k(f); (b = e.parseXML(f.responseText)) && b.firstChild ? (f = e.extend({},
				f, {
					responseXML: b
	...
	e.getCookies = function() {
		for (var f = {},
		a = j.cookie.split("; "), k = 0; k < a.length; k++) {
			var m = a[k].split("\x3d");
	...
	e.translateEventResponse = function(f, a) {
		...
		e.foreach(["position", "duration", "offset"],
		function(f, m) {
	...
	e.deepReplaceKeyName = function(f, a, k) {
		...
		case "object":
			...
			function(m, b) {
				...
				for (var d = m,
				c = 0; c < e.length; c++) d = d.replace(RegExp(a[c], "g"), k[c]);
	...
	e.repo = function() {
		...
		var f = "http://www.to8to.com/gb_php/";
		return f
	};
	...
	e.filterPlaylist = function(a, b, k) {
		...
		g,
		j;
		for (d = 0; d < a.length; d++) if (c = e.extend({},
		a[d]), c.sources = e.filterSources(c.sources, !1, k), 0 < c.sources.length) {
			for (g = 0; g < c.sources.length; g++) j = c.sources[g],
			j.label || (j.label = g.toString());
			m.push(c)
		}
		if (b && 0 === m.length) for (d = 0; d < a.length; d++) if (c = e.extend({},
		a[d]), c.sources = e.filterSources(c.sources, !0, k), 0 < c.sources.length) {
			for (g = 0; g < c.sources.length; g++) j = c.sources[g],
			j.label || (j.label = g.toString());
			m.push(c)
		}
...
function(h) {
	...
	var q = b.css = function(b, m, f) {
		...
		var h = !1,
		q, p;
		for (q in m) p = l(q, m[q], f),
		"" !== p ? p !== j[q] && (j[q] = p, h = !0) : void 0 !== j[q] && (delete j[q], h = !0);
		if (h) {
			if (!e[b]) {
	...
	h.scriptloader = function(j) {
		...
		this.load = function() {
			if (a == c.NEW) {
				...
				f.addEventListener ? (f.onload = b, f.onerror = g) : f.readyState && (f.onreadystatechange = function(a) { ("loaded" == f.readyState || "complete" == f.readyState) && b(a)
				});
...
function(h) {
	h.key = function(c) {
		...
		try {
			...
			var b = c.split("/"); (l = b[0]) ? /^(free|pro|premium|enterprise|ads)$/i.test(l) ? (j = b[1], b[2] && 0 < parseInt(b[2]) && (g = new Date, g.setTime(String(b[2])))) : l = "invalid": l = "free"
		} catch(e) {
			l = "invalid"
		}
	}
...
function(h) {
	...
	c.encrypt = function(g, b) {
		...
		for (var n = c.strToLongs(j.encode(b).slice(0, 16)), a = e.length, d = e[a - 1], f = e[0], h, k = Math.floor(6 + 52 / a), m = 0; 0 < k--;) {
			m += 2654435769;
			h = m >>> 2 & 3;
			for (var r = 0; r < a; r++) f = e[(r + 1) % a],
			d = (d >>> 5 ^ f << 2) + (f >>> 3 ^ d << 4) ^ (m ^ f) + (n[r & 3 ^ h] ^ d),
			d = e[r] += d
		}
		e = c.longsToStr(e);
		return l.encode(e)
	...
	c.decrypt = function(g, b) {
		...
		for (var e = c.strToLongs(l.decode(g)), h = c.strToLongs(j.encode(b).slice(0, 16)), a = e.length, d = e[a - 1], f = e[0], q, k = 2654435769 * Math.floor(6 + 52 / a); 0 != k;) {
			q = k >>> 2 & 3;
			for (var m = a - 1; 0 <= m; m--) d = e[0 < m ? m - 1 : a - 1],
			d = (d >>> 5 ^ f << 2) + (f >>> 3 ^ d << 4) ^ (k ^ f) + (h[m & 3 ^ q] ^ d),
			f = e[m] -= d;
			k -= 2654435769
		}
		e = c.longsToStr(e);
	...
	var l = {
		...
		encode: function(c, b) {
			...
			m += "\x00";
			for (k = 0; k < m.length; k += 3) e = m.charCodeAt(k),
			h = m.charCodeAt(k + 1),
			a = m.charCodeAt(k + 2),
			d = e << 16 | h << 8 | a,
			e = d >> 18 & 63,
			...
			a = d >> 6 & 63,
			d &= 63,
			f[k / 3] = r.charAt(e) + r.charAt(h) + r.charAt(a) + r.charAt(d);
			f = f.join("");
		...
		decode: function(c, b) {
			...
			m = l.code;
			k = b ? j.decode(c) : c;
			for (var r = 0; r < k.length; r += 4) e = m.indexOf(k.charAt(r)),
...
function(h) {
	...
	h.plugin = function(j) {
		...
		c.extend(this, k);
		this.load = function() {
			if (n == c.loaderstatus.NEW) if (0 < j.lastIndexOf(".swf")) a = j,
			n = c.loaderstatus.COMPLETE,
			k.sendEvent(l.COMPLETE);
			else if (c.getPluginPathType(j) == c.pluginPathType.CDN) n = c.loaderstatus.COMPLETE,
			k.sendEvent(l.COMPLETE);
			else {
	...
	h.plugins.pluginloader = function(g, b) {
		...
		this.setupPlugins = function(a, b, d) {
			...
			function(g, j) {
				...
				q = n.getJS(),
				I = n.getURL();
				l && (e.plugins[l] = c.extend({},
				j), e.plugins[l].pluginmode = n.getPluginmode(), e.length++);
				try {
					if (q && b.plugins && b.plugins[I]) {
						...
						r.style.top = 0;
						r.style.zIndex = f + 10;
						m[k] = n.getNewInstance(a, c.extend({},
						b.plugins[I]), r);
						f++;
						a.onReady(d(m[k], r, !0));
						a.onResize(d(m[k], r))
...
function(h) {
	var c = h.item = function(l) {
		...
		for (b = 0; b < g.sources.length; b++) e = g.sources[b]["default"],
...
function(h) {
	...
	c.adprovider = function(j, g) {
		...
		d.updateModel = function(d, e) { (d.tag || d.newstate || d.sequence || d.companions) && c.log("received ad change:", d);
	...
	j.controller = function(a, d) {
		...
		function x(e, f) {
			...
			if (g.reconcile) {
				...
				var m = g.diff,
				h = K;
				if (!m.id || !g.appid || !g.pageUrl) m.id = c().id,
				g.appid = J.appid,
				g.pageUrl = Q,
				K = w = null;
		...
		function v(m) {
			...
			a.jwSetVolume = function(a) {
				g.exists(a) && (a = Math.min(Math.max(0, a), 100) | 0, R(a) && (a = Math.max(0, Math.min(a / 100, 1)), w.setReceiverVolumeLevel(a, I,
				function(a) {
		...
		function H(c) {
			if ("state" === c.type) {
				if (y && (c.diff.newstate || c.diff.position)) y.destroy(),
				y = null,
				d.setVideo(z),
...
function(h) {
	...
	a = h.embed = function(d) {
		function f() {
			...
			else if (!F) if ("string" === j.typeOf(u.playlist)) C = new h.playlist.loader,
			C.addEventListener(g.JWPLAYER_PLAYLIST_LOADED,
			function(a) {
				u.playlist = a.playlist;
				F = e;
				f()
			}),
			C.addEventListener(g.JWPLAYER_ERROR,
			function(a) {
				F = e;
				m(a)
			}),
...
function(h) {
	function c(b) {
		if (b.playlist) for (var c = 0; c < b.playlist.length; c++) b.playlist[c] = new g(b.playlist[c]);
			...
			var h = {};
			j.foreach(g.defaults,
			function(a) {
				l(b, h, a)
			});
			h.sources || (b.levels ? (h.sources = b.levels, delete b.levels) : (c = {},
			l(b, c, "file"), l(b, c, "type"), h.sources = c.file ? [c] : []));
			b.playlist = [new g(h)]
		}
	}
	function l(b, c, g) {
...
function(h) {
	...
	h.embed.download = function(j, g, b) {
		function e(a, b) {
			for (var d = l.querySelectorAll(a), e = 0; e < d.length; e++) c.foreach(b,
			function(a, b) {
				d[e].style[a] = b
			})
		}
...
function(h, c) {
	...
	h.api = function(l) {
		...
		p.addButton = function(a, b, c, d) {
			try {
				E[d] = c,
				t("jwDockAddButton", a, b, "jwplayer('" + p.id + "').callback('" + d + "')", d)
			} catch(e) {
		...
		k(f,
		function(a, b) {
			G[a] || (G[a] = [], u(g.JWPLAYER_PLAYER_STATE,
			function(b) {
				var c = b.newstate;
				b = b.oldstate;
				if (c == a) {
		...
		p.playerReady = function(a) {
			...
			u(g.JWPLAYER_MEDIA_META,
			function(a) {
				j.extend(H, a.metadata)
			});
			u(g.JWPLAYER_VIEW_TAB_FOCUS,
			function(a) {
