...
// */s*.html 类型检索（主要用于区域模版的新型检索）
function seachWord() {
    var url = document.domain;
  //  if (url.indexOf("www")>=0) url = 'http://haikou.lou86.com';
    var keyvalue =$.trim($('.h_bname').val());// 关键字
    window.location.href = '/loupan/?key='+keyvalue;
}

// */s*.html 类型检索（主要用于区域模版的新型检索/滑动检索）
function seachWords() {
    var url = document.domain;
  //  if (url.indexOf("www")>=0) url = 'http://haikou.lou86.com';
    var keyvalue =$.trim($('.h_bname').val());// 关键字
...
function searchBuild(){        
    if($('.h_bname')){
        ...
        if (thisurl.indexOf("www")>=0){
            ...
            domin = thisurl.substring(thisurl.indexOf("//")+2,thisurl.indexOf(".gxloushitong"));
        }
        url="http://"+domin+".gxloushitong.com/";
       ...
       // url="http://"+domin+".gxloushitong.com/";
        if(keyvalue.length<=0)
            ...
            if(keyvalue=='请输入楼盘名称'){
                ...
                url=url+"?key="+encodeURIComponent(keyvalue)+"";
            }
        }
        window.location.href=url;
    }
