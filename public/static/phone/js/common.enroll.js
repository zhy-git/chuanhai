...
function openwid4(tit,tits,frm,type) {  
    $("#make_tit_2").html(tit);  
    $("#make_tit_3").html(tits);  
    $("#make_tit_4").val(frm);  
    $(".frm_id").val(type);      
...
//共用弹窗
  ...
  $('#frm_up_12').ajaxForm({
    beforeSubmit: checkForm12, // 此方法主要是提交前执行的方法，根据需要设置
    success: complete12, // 这是提交后的方法
    dataType: 'json'   
  });
  function checkForm3(){
       layer.open({type: 2});
  ...
  function checkForm12(){
      var mobile = /^1[3|4|5|6|7|8|9]\d{9}$/;         
      if(!mobile.test($.trim($('#mobile_12').val()))){                                
         layer.open({content: '请您输入正确的手机号码',time: 2 ,style: 'background: rgba(185, 0, 0, 0.6); color:#fff; border:none;'});
        $('#mobile_12').focus(); 
        return false;
      }
 ...
 function complete(data){    
  ...
  function complete3(data){     
    ...
    if(data.status==1){
       layer.closeAll('loading');
       openwid4('楼盘问答','请输入正确的手机号码，我们专业的置业顾问将给您最满意的解答。',name+'-楼盘问答',7);
       return false;
    }else{
      layer.closeAll('loading');
      layer.open({content: data.info,time: 2 ,style: 'background: rgba(185, 0, 0, 0.6); color:#fff; border:none;' });      
...
$(function(){
  ...
  function checkForm(){
        ...
        if(!mobile.test($.trim($('#find-mobile-1').val()))){            
  ...
  function checkForm2(){
        var mobile = /^1[3|4|5|7|8]\d{9}$/;        
        if(!mobile.test($.trim($('#find-mobile-2').val()))){            
         layer.open({content: '手机号格式非法',time: 2 ,style: 'background: rgba(185, 0, 0, 0.6); color:#fff; border:none;'});
          $('#find-mobile-2').focus(); 
          return false;
        }
