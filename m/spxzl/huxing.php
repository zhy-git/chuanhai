...
<?php include("../top.php");?>
		...
		<section class="wrap" style="background:#EFF3F6;">
			...
			<div class="loup_head swiper-container" id="swiper-container1">
			    <ul class="swiper-wrapper">
				   ...
				   ?> <li class="swiper-slide" style="position: relative;">
			            ...
			            <video id="vclick" src="<?php echo $site2.$infos['src7'];?>" autobuffer poster="<?php echo $site2.$infos['src8'];?>"  width="100%" height="100%" style="object-fit:fill" preload="none"></video>
			        </li>
                    <?php }?>
			        <li class="swiper-slide">
			            <a class="a" href="xiangce.php?lpid=<?php echo $lpid;?>"> <img src="<?php echo $site2.$infos['img'];?>" alt=""></a>
			            <div class="Number">
		    ...
		    <div class="hlsnav" style="margin:0px">
			   <ul>
			      <li><a href="index.php?lpid=<?php echo $lpid;?>"><span></span>楼盘</a></li>
			      <li><a href="news.php?lpid=<?php echo $lpid;?>"><span></span>动态</a></li>
			      <li><a href="xiangce.php?lpid=<?php echo $lpid;?>"><span></span>相册</a></li>
			      <li><a href="huxing.php?lpid=<?php echo $lpid;?>&pid_flag=xc1" class="on"><span></span>户型</a></li>
			  ...
			  <!-- 楼盘首页菜单结束 -->
		        ...
		        <!-- 楼盘详细信息 A-->
		    <style>
		    .new_hotelspic-list{width:100%;overflow:hidden;}
		    .new_hotelspic-list ul li{border:1px solid #EEE;width:42%;height:22%;margin:5px 5px;float:left; position:relative;left:5%;overflow:hidden;}
		    .new_hotelspic-list ul li h1{text-align:left;font-size:0.85em;line-height:27px;color:#646464;}
		    .new_hotelspic-list ul li img{width:100%;height:75%}
		    .optionMore .hotelspic-tabs{background:white;border:0px;} 
		  </style>
		    
			<div class="hotelspic">
  <div class="hotelspic-tabs">
    <ul>
      <?php
	 ...
	 ?>
       <li ><A href="huxing.php?pid_flag=xc1&lpid=<?php echo $lpid;?>&pid_hx=<?php echo $xclist['id'];?>"  class="<?php if($pid_hx==$xclist['id']){ echo ' on';}?>"><?php echo $xclist['flag'];?></A></li>
      <?php
      }
      }
	  ?>
  ...
  <div class="new_hotelspic-list">
    <ul id="">
          <?php
	 		if($pid_hx==''){
				$row = $mysql->query("select * from `web_pic` where `luopan_id`='{$lpid}' and `pid_flag`='xc1' order by id desc");
				//print_r($row);
				}else{$row = $mysql->query("select * from `web_pic` where `luopan_id`='{$lpid}' and `pid_flag`='xc1' and `pid_hx`='{$pid_hx}' order by id desc");
					}
...
<li data-src="<?php echo $site;?><?php echo $list['pic_img'];?>" data-sub-html="<?php echo $list['pic_name'];?>">
        
          <img src="<?php echo $site;?><?php echo $list['pic_img'];?>" style="height:140px;">
          <h1><?php echo $list['pic_name'];?></h1>
...
<script type="text/javascript">
      $(window).scrollTop($('.hlsnav').offset().top - 50);
      $('.new_hotelspic-list ul img').on('click',function(){
            $(this).parent().parent().attr('id','lightgallery');
           $('#lightgallery').lightGallery();

             var Getoption = $('.hotelspic-tabs');
             var phototime = 1;
             var photo_dsq = setInterval(function(){
                  phototime--;
                   if(phototime <= 0){
                       clearInterval(photo_dsq); 
                       $('.optionMore').html('');
                       $('.optionMore').append(Getoption.clone());
                       $('.lg-outer').removeClass('lg-thumb-open');
                       $('.lg-toogle-thumb').css({'top':'-80px'})
                       $('.lg-toogle-thumb').attr('id','option_tc');
                       newlg_toogle();
                   }
              },300)
      })
        ...
        function newlg_toogle(){
              $('.lg-toogle-thumb').on('click',function(){

                    if($(this).attr('id') == 'option_tc'){
                         $(this).css({'top':'-38px'});
                         $(this).attr('id','').attr('id','option_sq');
                    }else if($(this).attr('id') == 'option_sq'){
                         $(this).css({'top':'-80px'});
                         $(this).attr('id','').attr('id','option_tc');
...
$(function(){
    ...
    $('.hotelspic-tabs ul li').each(function(){
      var ain = $(this).find('a').html();
      $(this).attr('onclick','counts("'+ain+'")');

    })
})
function counts(i){
   /*页面点击统计*/
   _czc.push(﻿["_trackEvent","楼盘详细","楼盘相册",i,0,$('.copy_option')]);
}
