		...
		<section class="wrap" style="background:#EFF3F6;">
		    ...
		    <div class="hlsnav" style="margin:0px">
			   <ul>
			      ...
			      <li><a href="xiangce.php?lpid=<?php echo $lpid;?>" class="on"><span></span>相册</a></li>
  ...
  <div class="hotelspic-tabs">
    <ul>
		...
		?>
            ...
            <li><a href="?lpid=<?php echo $lpid;?>&pid_flag=<?php echo $list['flag_bm'];?>" class="<?php if($pid_flag==$list['flag_bm']){ echo ' on';}?>"><?php echo $list['flag'];?></a></li>
        <?php
			}
		?>
            <div class="clear"></div>
    ...
    <ul id="">
          <?php
	 		if($pid_flag==''){
				...
				//print_r($row);
				}else{$row = $mysql->query("select * from `web_pic` where `luopan_id`='{$lpid}' and `pid_flag`='{$pid_flag}' order by id desc");
					}
