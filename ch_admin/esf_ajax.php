...
if($result["total"]==0){
		...
		foreach($row as $list){
			...
			if($list['esfzt']==1){
				$esfzt='<font color="#FF0000">销售中</font>';
				}else{
					$esfzt='<font color="#009900">已售出</font>';
					}
			$data['html'].='<tr class="tr '.$lv.'" style="">
					...
					<table width="100%" border="0" style="background:#FFF;">
					  <tr>
						...
						<td style="padding:5px 0px;"><a style="color:#0CF; font-size:18px; " href="esf_add.php?id='.$list[id].'&pid='.$list[pid].'">'.$list[title].'</a>'.$flag.' '.$flagztz.'</td>
						<td style="width:15%;">价格：'.$list[tuanprice].'</td>
						<td style="padding:5px 0px;width:25%;" align="center">'.$rowcity[0][city_name].'-'.$rowcity2[0][city_name].'</td>
					  </tr>
					  <tr>
						<td style="padding:5px 0px;">户型：'.$list[esfhx1].'室'.$list[esfhx2].'厅'.$list[esfhx3].'厨'.$list[esfhx4].'卫'.$list[esfhx5].'阳台　　面积：'.$list[jzmj].'</td>
						<td>年代：'.$list[esfnd].'年</td>
					...
					<td><a href="esf_add.php?id='.$list[id].'&pid='.$list[pid].'" target="c" class="ceditor" style=" margin-right:5px;">详细信息</a></td>
					</tr>';
			$i++;
			}
		}
