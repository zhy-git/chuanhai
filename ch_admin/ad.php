    ...
    <script>
        $(document).ready(function() {
            ...
            $('#dely1').click(function(event) {
                ...
                if (delid=='') {
                    ...
                    $.ajax({
                        ...
                        dataType:'JSON',
                        type:'POST',
                        data:'delid='+delid+'&delid1='+delid1+'&action=get_del',
            ...
            function clientajax(keyword) {
                ...
                $.get('ad_ajax.php?page='+keyword+'&keys='+keys+'&pid='+pid+'&ad_id='+ad_id+'&city_id='+city_id, function(data) {
                    //alert(data.html);
                    //回调函数
                    //data = eval('('+data+')');
                    if (data.msg=='ok') {
...
<div class="nav_guide">当前位置：<?php echo get_srot($pid,' &gt; ').$edit_title;?>
    <div class="cus_search2"><a href="ad_add.php?pid=<?php echo $pid;?>&ad_id=<?php echo $ad_id;?>" class="newadd">添加新信息</a></div>
</div>
<div class="cus_search c">
    ...
    <div class="cus_tiem" style="line-height:40px;">
        ...
        //    echo $name =  $cityall['id'];
        if ($city_id == $cityall['id']) {
            $cy = ' style=" color:#F00;"';
        } else {
            $cy = '';
        }
        ...
        /*  echo "<div class='cus_tiem'>";
          echo " 广告分类：";
          $row = $mysql->query("select * from `web_srot` where `pid`='{$pid}' and `startus`='1'");
          foreach ($row as $list) {
              */?><!--
                        <a href="?pid=<?php /*echo $pid; */?>&ad_id=<?php /*echo $list['id']; */?>&city_id=<?php /*echo $cityall['id']; */?>" <?php /*if ($ad_id == $list['id']) {
    ...
    <div class="cus_tiem">
        ...
        foreach($row as $list){
            ?>
            <a href="?pid=<?php echo $pid;?>&ad_id=<?php echo $list['id'];?>&city_id=<?php echo $city_id;?>" <?php if($ad_id==$list['id']){ echo ' style="color:#F00"';}?>><?php echo $list['title'];?></a>　　
            <?php
        }
