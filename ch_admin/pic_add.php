...
<div class="nav_guide">当前位置：楼盘相册管理 &gt; 楼盘[<font color="#0099FF" style="font-size:16px;"><?php echo $lpname;?></font>]相册信息
	<div class="cus_search2"><a href="loupan_pic.php?lpid=<?php echo $lpid;?>&pid=<?php echo $pid;?>" class="back">返回楼盘相册列表</a></div>
</div>
<div class="content2">
	<div class="searchinfor c" style="position:relative; min-height:500px;">
...
function challs_flash_update(){ //Flash 初始化函数
	...
	a.FormID=['select','select2','select3'];
...
function show_xc(a){
	...
	<div style="width:408px; height:348px; position:absolute; top:10px; left:10px;"><div style="background:#DCE5F1; border:#5576B8 solid 1px; font-size:12px; line-height:22px; text-align:left;">
	    ...
	    <option value="<?php echo $lpid;?>"><?php echo $lpname;?></option>
      </select>
      <select id="select2" name="select2" style="position:absolute; top:0px; left:172px;" onChange="show_xc(this.value);">
     <?php
	 $row = $mysql->query("select * from `web_flag` where `flag_fl`='9' and `flag_st`='1' order by `flag_px` asc");
     foreach($row as $list){
	 ?>
	    <option value="<?php echo $list['flag_bm'];?>"><?php echo $list['flag'];?></option>
      <?php
      }
	  ?>
      </select>
      <select id="select3" name="select3" style="position:absolute; top:0px; left:272px;">
     <?php
	 $row = $mysql->query("select * from `web_flag` where `flag_fl`='41' order by `flag_px` asc");
     foreach($row as $list){
	 ?>
	    <option value="<?php echo $list['id'];?>"><?php echo $list['flag'];?></option>
      <?php
      }
	  ?>
      </select>
      　　</div></div>
<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=10,0,0,0" width="408" height="600" id="update" align="middle">
<param name="allowFullScreen" value="false" />
    ...
    <div style="width:100px; height:16px; position:absolute; bottom:5px; left:20px; background:#DCE5F1;"></div>
    <div style="width:222px; height:16px; position:absolute; top:30px; left:90px;background:#DCE5F1; z-index:999">图片上传大小：宽800px*高600px</div>
    </div>
    
<div id="show" style="width:500px; height:630px; overflow-y:auto; text-align:left; position:absolute; top:10px; left:450px;background:#DCE5F1; border:#5576B8 solid 1px; font-size:12px;">信息提示框！<br />
</div>
