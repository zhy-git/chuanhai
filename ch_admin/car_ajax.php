...
if($result["total"]==0){
		...
		foreach($row as $list){
			...
			foreach($rowflag as $listflag){
				if(preg_match("#".$listflag['flag_bm']."#", $list['flagts'])){
					$flagts.='<font color="'.$listflag['color'].'">['.$listflag['flag'].'] </font>';
				}
			}
			 $flaglb='';
			  $rowflag = $mysql->query("select * from `web_flag` where `flag_fl`='17' and `flag_st`='1' order by `flag_px` asc ");
			foreach($rowflag as $listflag){
				if(preg_match("#".$listflag['flag_bm']."#", $list['flaglb'])){
					$flaglb.='<font color="'.$listflag['color'].'">['.$listflag['flag'].'] </font>';
				}
			}
			...
			$data['html'].='<tr class="tr '.$lv.'" style="">
					...
					<table width="100%" border="0" style="background:#FFF;">
					  <tr>
						...
						<td style="padding:5px 0px;"><a style="color:#0CF; font-size:18px; " href="car_add.php?id='.$list[id].'&pid='.$list[pid].'">'.$list[title].'</a>'.$flag.' '.$flagts.' '.$flaglb.'</td>
						<td style="width:15%;">日均价：'.$list[tuanprice].'元/天</td>
						<td style="padding:5px 0px;width:25%;" align="center">'.$rowcity[0][city_name].'</td>
					  </tr>
					  <tr>
						<td style="padding:5px 0px;">参数：'.$list[sldz].'</td>
						<td>原价：'.$list[tuanprice2].'元/天</td>
						<td style="padding:0px;" align="center"> </td>
					  </tr>
					  <tr>
						<td style="padding:5px 0px;" colspan="2">优惠：'.$list[xmdz].'</td>
						<td style="padding:0px;"></td>
					  </tr>
					</table>
					</td>
					<td><a href="car_add.php?id='.$list[id].'&pid='.$list[pid].'" target="c" class="ceditor" style=" margin-right:5px;">详细信息</a></td>
					</tr>';
			$i++;
			}
		}
