...
if($result["total"]==0){
		...
		foreach($row as $list){
			...
			$data['html'].='<tr class="tr '.$lv.'" style="">
					...
					<td><a style="" href="link_add.php?id='.$list[id].'&pid='.$list[pid].'">'.$list[title].'</a></td>
					<td>'.$list[link_url].'</td>
					<td><a href="link_add.php?id='.$list[id].'&pid='.$list[pid].'" target="c" class="ceditor" style=" margin-right:5px;">详情</a></td>
					</tr>';
			$i++;
			}
		}
