...
$keys=$_POST['keys'];
$lpid=$_GET['lpid'];
$rowlp=$mysql->query("SELECT * FROM `web_content_dp` WHERE `id`='{$lpid}'");
...
$(document).ready(function() {
		...
		$('#dely1').click(function(event) {
			...
			if (delid=='') {
				...
				$.ajax({
					...
					dataType:'JSON',
					type:'POST',
					data:'delid='+delid+'&delid1='+delid1+'&action=get_infodpdel',
	...
	function clientajax(keyword) {
		...
		$.get('loupan_dp_ajax.php?page='+keyword+'&keys='+keys+'&pid='+pid+'&lpid='+lpid, function(data) {
			//alert(data.html);
			//回调函数
			//data = eval('('+data+')');
			if (data.msg=='ok') {
...
<body>
<div class="nav_guide">当前位置：<?php echo get_srot($edit_pid,' &gt; ').$edit_title;?>
<div class="cus_search2"><a href="loupan_dp_add.php?pid=<?php echo $pid;?>&lpid=<?php echo $lpid;?>" class="newadd">添加楼盘点评</a><a href="loupan_list.php?pid=<?php echo $rowlp[0]['pid'];?>&page=<?php echo $_GET['page'];?>" class="back">返回楼盘列表</a></div>
</div>
<div class="cus_search c">
  <form name="myform" action="?pid=<?php echo $pid;?>&lpid=<?php echo $lpid;?>" method="post">
    <div class="cus_tiem">
