...
if($_GET['ad_id']<>""){
	$rowc=$mysql->query("SELECT * FROM `web_srot` WHERE `id`='{$_GET['ad_id']}'");
	}else{
	 ...
	 if($infos==false)
	  {
		...
		exit;
		   ...
		   $ad_id=$infos['ad_id'];
		   $rowa=$mysql->query("SELECT * FROM `web_srot` WHERE `id`='{$ad_id}'");
		   	$infoa=$rowa[0];
			 if($infoa==false)
			  {
				echo "已经出错!";
...
//					}else{
						showimg.html("<img src='../"+img+"' id='cropbox'  style='height:65px;' />");
//					}
					//传给php页面，进行保存的图片值
					$("#src").val(img);
					//截取图片的js
...
<div class="content">
  <form action="submit_save.php?action=ad_add" method="post" name="myform" id="myform">
    <div class="addcust" id="client">
      <ul class="addcust_ul c">
        <li>
          <p class="lie1">
            ...
            <select name="ad_pid" class="select" style=" height:34px;" id="ad_pid">
					  ...
					  foreach($row as $list){
					  ?>
                    <option value="<?php echo $list['id'];?>" alt="<?php echo $list['wh'];?>" id="level" <?php if($ad_id==$list['id']){ echo 'selected="selected"';}?>><?php echo $list['title'];?></option>
                      <?php
					  }
					  ?>
                   </select>
        ...
        <li>
          <p class="lie1">
            <label>图片大小：</label><font id="wh"><?php echo $p_wh;?></font>
          </p>
        </li>
        <li>
          <p>
            ...
            <span class="upid">
              <span class="btn"><span>上传图片</span><input id="fileupload" type="file" name="mypic" /></span>
                ...
                <span id="showimg"><img src="<?php if($infos['img']<>""){echo "../".$infos['img'];}else{echo 'images/01.jpg';}?>" style=" height:65px;" /><!--初始图片--></span>
