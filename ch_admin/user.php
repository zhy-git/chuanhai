...
$rsc = $mysql->query("select count(*) as count from ` web_user`");
$result["total"] = $rsc[0]['count'];
$total=ceil($result["total"]/$page_num);
$row = $mysql->query("select * from `web_user` {$sql} order by addtime desc limit $offset,$page_num");
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="box_cus" id="table">
	 ...
	 // print_r($row);
	  ...
	  echo '<tr class="tr '.$lv.'" style="">
					...
					'.$lpmc.'
					<td style="text-align:left;">'.$list[uname].'</td>
					<td style="text-align:left;">'.$list[u_rname].'</td>
					<td style="text-align:left;">'.$list[u_tel].'</td>
					<td style="text-align:left;">'.$list[u_mail].'</td>
					<td>'.$list[addtime].'</td>
					</tr>';
