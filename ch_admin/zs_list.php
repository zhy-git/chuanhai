...
$(document).ready(function() {
		...
		$('#dely1').click(function(event) {
			...
			if (delid=='') {
				...
				$.ajax({
					...
					dataType:'JSON',
					type:'POST',
					data:'delid='+delid+'&delid1='+delid1+'&action=get_zsdel',
	...
	function clientajax(keyword) {
		...
		$.get('zs_ajax.php?page='+keyword+'&keys='+keys+'&pid='+pid+'&city_id='+city_id, function(data) {
			//alert(data.html);
			//回调函数
			//data = eval('('+data+')');
			if (data.msg=='ok') {
...
<div class="cus_search2"><a href="zs_add.php?pid=<?php echo $pid;?>" class="newadd">添加新信息</a></div>
</div>
